...
const ProductDetails = () => {
    ...
    const renderStars = (rating: number) => {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(<FontAwesomeIcon icon={faStar} key={`full-${i}`} className="h-5 w-5 text-yellow-400" />);
        }
        ...
        for (let i = 0; i < emptyStars; i++) {
            stars.push(<FontAwesomeIcon icon={faStar} key={`empty-${i}`} className="h-5 w-5 text-gray-300" />);
        }
